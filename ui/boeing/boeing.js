 ...
 * @class
 */
function BoeingUIExt() {
    var that = this;

    this._skinName = "boeing";
    ...
    this.ToggleAudioText = ds.utils.ExtendMethod(this.ToggleAudioText, function () {
        var color = this.audioTextEnabled() ? this.config.controlBarBottomActiveIconColor() : this.config.controlBarBottomIconColor();
        $(".fa-closed-captioning").css("color", color);
    });

    this.InitTitleIcon = function () {
        var link = document.createElement('link');
        link.rel = "icon";
        link.type = "image/x-icon";
        link.href = "images/favicon_boeing.ico";
        document.head.appendChild(link);
    };

    this.InitNextCompletedCss = function (activeIconColor) {
        var style = document.createElement('style');
        style.textContent =
            ".nextCompleted {" +
            "animation:colorLoop 0.75s infinite;" +
            "-moz-animation:colorLoop 0.75s infinite; /* Firefox */" +
            "-webkit-animation:colorLoop 0.75s infinite; /* Safari and Chrome */" +
            "-o-animation:colorLoop 0.75s infinite; /* Opera */" +
            "}" +

            "@keyframes colorLoop {" +
            "0% {" +
            "color:#fff;" +
            "background-color:" + activeIconColor + ";" +
            "border-color:" + activeIconColor + ";" +
            "}" +
    ...
    this.InitTopicColorCss = function (topicColor) {
        var style = document.createElement('style');
        style.textContent =
            ".cmenu-item.cmenu-group .item-text," +
            ".text-topic {" +
            "color: " + topicColor + ";" +
            "}" +

            ".cmenu-item .collapse-icon {" +
            "color: " + topicColor + ";" +
            "}" +

            ".cmenu-item.isCurrent:after {" +
            "border-color: " + topicColor + ";" +
            "}" +

            ".cmenu-item.selected:before {" +
    ...
    */
    ...
    this.GetContentHeight = function () {
        ...
        var frameHeight = that.audioTextEnabled() && that.pageController() && that.pageController().audioText() ? $container.height() - 114 : $container.height() - 12;
        return frameHeight;
    };
     ...
     */
    this.GetAboutPageCopyrightText = function () {
        var date = new Date();
        return "BOEING PROPRIETARY– Copyright © " + date.getFullYear() + " Boeing. <br /> <br />" +
               "All rights reserved. ECCN 9E991";
    };

    this.GetFooterCopyrightText = function () {
        var date = new Date();
        return "BOEING PROPRIETARY– Copyright © " + date.getFullYear() + " Boeing. All rights reserved. ECCN 9E991";
    };

    this.GetLoadingPageCopyrightText = function () {
        var date = new Date();
        return "Boeing Proprietary Copyright © " + date.getFullYear() + " Boeing. All rights reserved. <br /> <br />" +
               "EXPORT CONTROLLED - This technology or software is subject to the U.S. Export Administration Regulations (EAR), (15 C.F.R. Parts 730 - 774). " +
               "No authorization from the U.S. Department of Commerce is required for export, re-export, in-country transfer, or access EXCEPT to country group E:1 or E:2 countries/persons per Supp.1 to Part 740 of the EAR. ECCN 9E991";
    };
    ...
    this._InitUI = function () {
        document.title = "Boeing CBT Player";

        var ls = document.createElement("link");
        ls.rel = "stylesheet";
        ls.href = "ui/boeing/boeing.css";
        document.getElementsByTagName("head")[0].appendChild(ls);
        document.getElementById("appClosedLogo").src = "ui/boeing/images/btm_boeingco_logo.png";

        that.InitElements();
        that.InitComponents();
        that.InitTitleIcon();
        that.InitSubscriptions();
    ...
    /**
    * Init ko data subscriptions.
    */
    this.InitSubscriptions = function () {
        that.config.controlBarBottomActiveIconColor.subscribe(function (value) {
            if (value) {
                that.InitNextCompletedCss(value);
            }
        });
        that.config.controlBarBottomActiveIconColor.valueHasMutated();

        that.config.topicColor.subscribe(function (value) {
            if (value) {
                that.InitTopicColorCss(value);
            }
        });
        that.config.topicColor.valueHasMutated();
    };
    ...
    this.InitUIEventHandlers = function () {
        ...
        //pause when modals are shown
        var $modals = $('#help-modal');
        $modals.on('show.bs.modal', function () {
            that.Pause();
        });

        //play when modals are hidden
...
if (!ko.components.isRegistered('boeing')) {
    ko.components.register('boeing', {
        viewModel: {
